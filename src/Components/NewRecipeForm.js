...
export default function NewRecipeForm({ addNewRecipe }) {
  ...
  const [recipeImage, setRecipeImage] = useState(
    'https://thumbs.dreamstime.com/b/white-empty-dinner-plate-blue-border-wooden-table-93048343.jpg'
  );
  ...
  const removeIngredient = (e) => {
    e.preventDefault();
    setIngredients(
      ingredients.filter((ingredient) => ingredient.id !== ingredients.length)
    );
  };
  ...
  const removeDirection = (e) => {
    e.preventDefault();
    setDirections(
      directions.filter((direction) => direction.id !== directions.length)
    );
  };
  ...
  const handleSubmit = (e) => {
    ...
    ingredients.forEach((ingredient) =>
      setIngredientsArray(ingredientsArray.push(ingredient.text))
    );
    directions.forEach((direction) =>
      setDirectionsArray(directionsArray.push(direction.text))
    );
            ...
            {ingredients.map((row) => (
              <div key={row.id}>
                <input
                  ...
                  type='text'
                  name={`ingredient${row.id}`}
                  placeholder={`Ingredient ${row.id}`}
                ></input>
                <button onClick={(e) => addIngredient(e)}>Add</button>
                {row.id > 1 ? (
                  <button onClick={(e) => removeIngredient(e)}>X</button>
                ) : null}
              </div>
            ))}
          </div>
            ...
            {directions.map((row) => (
              <div key={row.id}>
                <input
                  ...
                  placeholder={`Step ${row.id}`}
                ></input>
                <button onClick={(e) => addDirection(e)}>Add</button>
                {row.id > 1 ? (
                  <button onClick={(e) => removeDirection(e)}>X</button>
                ) : null}
              </div>
            ))}
          </div>
